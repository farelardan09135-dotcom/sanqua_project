<x-admin-layout :title="'Customer'">
    ...
    <div x-data="{
            ...
            showEditModal: false,
            editMode: false,
            editingItem: { id: null, nama: '', no_wa: ''},
            openEdit(item, id) {
                this.editingItem = { id: id, nama: item.nama, no_wa: item.no_wa };
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="py-2 px-3">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('admin.customer.destroy', $item->id) }}" onsubmit="return confirm('Yakin hapus {{ $item->nama_customer }}?');">
                                            @csrf @method('DELETE')
                                            <button type="submit" :class="editMode ? 'opacity-100 scale-100' : 'opacity-40 scale-90 pointer-events-none'"
        ...
        <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
            ...
            <div x-show="showEditModal" class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                ...
                <form method="POST" :action="`/admin/customer/${editingItem.id}`" class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-600 mb-1">Nama Customer</label>
                        <input type="text" name="nama_customer" x-model="editingItem.nama" required class="w-full h-11 px-3 text-sm rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-600 mb-1">Kontak</label>
                        <input type="text" name="no_wa" x-model="editingItem.kontak" required class="w-full h-11 px-3 text-sm rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500">
                    </div>
                    <div class="flex items-center gap-3 pt-2">
