<x-admin-layout :title="'Customer'">
    ...
    <h1 class="text-2xl font-bold text-blue-700 mb-6">Customer</h1>
    ...
    <div x-data="{
        ...
        editMode: false,
        editingItem: { id: null, nama_customer: '', no_wa: '' },
        deleteTarget: { id: null, nama: '' },
        openEdit(item, id) {
            this.editingItem = { id: id, nama_customer: item.nama_customer, no_wa: item.no_wa };
        ...
        <div class="flex items-center justify-between gap-4 mb-6">
            <form method="GET" action="{{ route('admin.customer') }}" class="flex items-center gap-2 w-full max-w-md h-10 px-4 rounded-xl bg-white border border-slate-200 shadow-sm focus-within:ring-2 focus-within:ring-blue-500/40 focus-within:border-blue-500 transition-all">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-5">
            <h2 class="text-base font-bold text-slate-800 mb-3">Daftar Customer</h2>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-slate-200 text-slate-500">
                            ...
                            <th class="text-left font-semibold py-2 px-3">Nama Customer</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($customers as $index => $item)
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="py-2 px-3 text-slate-500">{{ $customers->firstItem() + $index }}</td>
                                <td class="py-2 px-3 font-medium text-slate-800">{{ $item->nama_customer }}</td>
                                <td class="py-2 px-3 text-slate-500">{{ $item->no_wa }}</td>
                                <td class="py-2 px-3">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <button type="button" @click="openDelete({{ $item->id }}, @js($item->nama_customer))"
                                            :class="editMode ? 'opacity-100 scale-100' : 'opacity-40 scale-90 pointer-events-none'"
                        ...
                        @empty
                            <tr><td colspan="5" class="py-8 text-center text-slate-400 italic">Belum ada data Customer.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div x-show="showAddModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
            ...
            <div x-show="showAddModal" class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-bold text-slate-800">Tambah Customer</h3>
                ...
                <form method="POST" action="{{ route('admin.customer.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_customer" required class="w-full h-11 px-3 text-sm rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-600 mb-1">Kontak</label>
                        <input type="text" name="no_wa" required class="w-full h-11 px-3 text-sm rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500" placeholder="08xx...">
                    </div>
        ...
        <div x-show="showEditModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
            ...
            <div x-show="showEditModal" class="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                ...
                <form method="POST" :action="`/admin/customer/${editingItem.id}`" class="space-y-4">
                    @csrf @method('PUT')
                    <div>
                        <label class="block text-sm font-medium text-slate-600 mb-1">Nama Customer</label>
                        <input type="text" name="nama_customer" x-model="editingItem.nama_customer" required class="w-full h-11 px-3 text-sm rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-600 mb-1">Kontak</label>
                        <input type="text" name="no_wa" x-model="editingItem.no_wa" required class="w-full h-11 px-3 text-sm rounded-xl bg-slate-50 border border-slate-200 focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500">
                    </div>
                    <div class="flex items-center gap-3 pt-2">
        ...
        <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" style="display: none;">
            ...
            <div x-show="showDeleteModal" class="relative bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
                ...
                <h3 class="text-base font-bold text-slate-800 mb-1">Hapus Customer?</h3>
                ...
                <form :action="`/admin/customer/${deleteTarget.id}`" method="POST" class="flex items-center gap-3">
                    @csrf @method('DELETE')
